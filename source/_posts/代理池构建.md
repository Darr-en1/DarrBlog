...
该项目主要针对企业级代理池设计实现，基于 [https://github.com/Python3WebSpider/ProxyPool](https://github.com/Python3WebSpider/ProxyPool)，作者是[崔庆才](https://cuiqingcai.com/)，是我非常喜欢的一个技术大拿，他所著作的《书籍python3网络爬虫》真的既详细又有深度，非常推荐。

该项目源码地址：[https://github.com/Darr-en1/ProxyPool](https://github.com/Darr-en1/ProxyPool)

**优势：项目对原有项目进行了重构，使用async 异步编程，引入fastapi，整个服务的性能得到显著提升**
...
class BaseCrawler:
    ...
    async def fetch(self, url, **kwargs):
        ...
        async with RetryClient(raise_for_status=RAISE_FOR_STATUS, retry_options=RETRY_OPTION) as client:
    ...
    def parse(self, html) -> List[Proxy]:
    ...
    def parse(self, html):
        ...
        for addr in hosts_ports:
            if (addr):
                ...
                proxy_list.append(Proxy(host=host, port=port, expire=int(time.time()) + 60 * 3))
        return proxy_list
```
...
class Tester:
    ...
    async def test(self, proxy: Proxy, sem: asyncio.Semaphore):
        """
        test single proxy
